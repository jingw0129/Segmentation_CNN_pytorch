...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    vgg_layer7_logits = tf.compat.v1.layers.conv2d(
        ...
        kernel_regularizer= tf.keras.regularizers.l2(0.5 * (1e-4)), name='vgg_layer7_logits')
    vgg_layer4_logits = tf.compat.v1.layers.conv2d(
        vgg_layer4_out, num_classes, kernel_size=1,
        kernel_initializer= tf.compat.v1.random_normal_initializer(stddev=0.01),
        kernel_regularizer= tf.keras.regularizers.l2(0.5 * (1e-4)), name='vgg_layer4_logits')
    vgg_layer3_logits = tf.compat.v1.layers.conv2d(
        vgg_layer3_out, num_classes, kernel_size=1,
        kernel_initializer= tf.compat.v1.random_normal_initializer(stddev=0.01),
        kernel_regularizer= tf.keras.regularizers.l2(0.5 * (1e-4)), name='vgg_layer3_logits')
    ...
    fcn_decoder_layer1 = tf.compat.v1.layers.conv2d_transpose(
        ...
        padding='same',
        kernel_initializer= tf.compat.v1.random_normal_initializer(stddev=0.01),
        kernel_regularizer= tf.keras.regularizers.l2(0.5 * (1e-4)), name='fcn_decoder_layer1')
    ...
    fcn_decoder_layer3 = tf.compat.v1.layers.conv2d_transpose(
        ...
        padding='same',
        kernel_initializer= tf.compat.v1.random_normal_initializer(stddev=0.01),
        kernel_regularizer= tf.keras.regularizers.l2(0.5 * (1e-4)), name='fcn_decoder_layer3')
    ...
    fcn_decoder_output = tf.compat.v1.layers.conv2d_transpose(
        ...
        padding='same',
        kernel_initializer= tf.compat.v1.random_normal_initializer(stddev=0.01),
        kernel_regularizer= tf.keras.regularizers.l2(0.5 * (1e-4)), name='fcn_decoder_layer4')
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cross_entropy_loss = tf.reduce_mean(input_tensor=tf.nn.softmax_cross_entropy_with_logits(logits= logits, labels= tf.stop_gradient( correct_label)))
...
def run():
    ...
    data_dir = './data'
    runs_dir = './runs'
    helper.test_for_kitti_dataset(data_dir)

    # Download pretrained vgg model
    tests.maybe_download_pretrained_vgg(data_dir)
    ...
    with tf.compat.v1.Session() as sess:
        ...
        get_batches_fn = tests.gen_batch_function(os.path.join(data_dir, 'data_road/training'), image_shape)
        ...
        tests.save_inference_samples(runs_dir, data_dir, sess, image_shape, logits, keep_prob, input_image)


def predict_images(test_data_path, print_speed=False):
    num_classes = 2
    image_shape = (160, 576)
    runs_dir = './data_road/testing/image_2/'
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # Predict the samples
        tests.pred_samples(runs_dir, test_data_path, sess, image_shape, logits, keep_prob, input_image, print_speed)
...
if __name__ == '__main__':
    ...
    if training_flag:
        ...
        tests.maybe_download_pretrained_vgg('vgg_model/')
        helper.test_layers(layers)
        helper.test_optimize(optimize)
        helper.test_train_nn(train_nn)
        ...
        test_data_path = '//DESKTOP-ASB277M/Data_3D/Training/3D_data/test/'
